...
I want to describe in a single structure and easy to understand, all the [] of a type, like:
 * how it related a node with its parent node
 * how is declared
 * what is referencing
 * etc

 IMPORTANT: I'm not being 

Given a node, I want to extract all this information from it and from its type so I can show it to the user.

Motivation: AST viewers, syntax highlight, type node serialization. etc

## ts simple ast Type.getStructure

This is an idea for a project. In ts-simple-ast nodes can be serialized to a structure - simplified - so declaration and
serialization of them can be done programmatically. 

But Type does not support this. I was designing a similar solution, perhaps it can be jsut implemented in ts-simple-ast
as a feature:
...
/**
 * How is this node associated with its parent?
 */
...
export function buildDeclarationKind({ node, type, project }: BuildMemberOptions): DeclarationKind[] {
...
export function buildBaseKind({ node, type, project }: BuildMemberOptions): TypeBaseKind[] {
